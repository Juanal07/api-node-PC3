...
def examenJavier(lugar):
    url = 'https://www.20minutos.es/busqueda//?q='+lugar+'&sort_field=&category=&publishedAt%5Bfrom%5D=&publishedAt%5Buntil%5D='
    page = requests.get(url)
    soup = BeautifulSoup(page.content, 'html.parser')
    links = soup.find("section", attrs={"class": "content content-100"})
    links1 = links.find_all('div', class_='media-content')
    ...
    for item1 in links1:
        ...
        page = requests.get(link)
        soup = BeautifulSoup(page.content, 'html.parser')
        titular = soup.find('h1', class_='article-title')
        titular = titular.text
        titular = re.sub('"','',titular) #para quitar las comillas en el texto
        fecha = soup.find('span', class_='article-date')
        fecha = fecha.text
        texto = soup.find('div', class_="article-text")
        texto = soup.find_all('p', class_='paragraph')
        ...
        for  textos in texto:
            total += textos.text
        total = re.sub('"','',total) #para quitar las comillas en el texto
        json += ('{"titular": "'+str(titular)+'","fecha": "'+str(fecha)+'","texto": "'+str(total)+'","enlace": "'+str(link)+'"},')
    json = json[:-1] #quito la ultima coma del json
    json += ']'
    print(json)

try:
    examenJavier(sys.argv[1])
except:
    print('{"titular": "error","fecha": "error","texto": "error","enlace": "error"}')
sys.stdout.flush()